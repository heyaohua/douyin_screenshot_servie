...
async def test_long_screenshot_service():
    """测试长截图服务"""
    
    logger.info("=" * 60)
    logger.info("开始测试长截图服务")
    logger.info("=" * 60)
    
    try:
        # 初始化浏览器
        logger.info("正在初始化浏览器...")
        if not await playwright_service.initialize():
...
if __name__ == "__main__":
    asyncio.run(test_long_screenshot_service())
